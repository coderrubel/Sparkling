        ...
        if (is_front_page() && of_get_option('sparkling_slider_checkbox') == 1) {
            ...
            if ($query->have_posts()) :
                while ($query->have_posts()) : $query->the_post();
                    ...
                    the_category();
                    if (get_the_title() != '')
                        echo '<h2 class="entry-title text-center">' . get_the_title() . '</h2>';
                    ?>

                    <div class="entry-meta-slider text-cente">
                        <i class="fa fa-clock-o icon" aria-hidden="true"></i> <?php the_time('t F  Y'); ?> 
                        <i class="fa fa-user icon" aria-hidden="true"></i> <?php the_author(); ?>
                    </div>
                    <?php
